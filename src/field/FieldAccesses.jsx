...
export function FieldAccesses({ profile }) {
  ...
  return <section className="field-accesses" aria-label="Logins da equipe e aplicativo">
    <header className="field-heading"><div><p className="eyebrow">VNA Comunidade / PAF</p><h2>Equipe e aplicativo</h2><p>{rows.filter(row => row.ativo).length} ativos / {rows.filter(row => !row.ativo).length} bloqueados</p></div><button className="primary-button" onClick={() => setEditor({})}><Plus size={18} />Cadastrar acesso</button></header>
