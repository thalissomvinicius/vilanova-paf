...
const settlementDetails = (isFederal, motherName) => [['Assentamento federal (INCRA)', settlementLabel(isFederal)], ...(isFederal === true ? [['Nome completo da mãe', motherName]] : [])];
...
export function LandAdmin({ api }) {
  ...
  function exportPage() {
    ...
    const rows = [['Protocolo', 'Nome', 'CPF', 'Nascimento', 'Telefone', 'Município', 'Comunidade', 'Assentamento federal (INCRA)', 'Nome completo da mãe', 'Resultado', 'Recebido em'], ...data.requests.map(row => [row.protocol, row.full_name, row.cpf, birthday(row.birth_date), row.phone, row.municipality, row.community, settlementLabel(row.is_federal_settlement), row.mother_name || '', LAND_STATUSES[row.status], date(row.created_at)])];
