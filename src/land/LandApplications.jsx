...
const settlementLabel = value => value === true ? 'Sim' : value === false ? 'Não' : 'Não informado';
...
function History({ request, history = [], staff = false }) {
  return <ol className="land-history">
    {history.map((item, index) => <li key={item.id || index}><Status value={item.status} /><time>{date(item.created_at)}</time><p>{item.comment}</p>{item.reviewer_name && <small>Análise realizada por: {item.reviewer_name}</small>}{staff && <small>{item.actor}</small>}</li>)}
...
export function LandPublic() {
  ...
  return <div className="land-public land-ui">
    ...
    <main className="land-public-main">
      <div className="land-intro"><p className="eyebrow">AGRICULTURA FAMILIAR</p><h1>Cadastro de Áreas para Análise - Plantio de Dendê</h1><p>Cadastre seu interesse e acompanhe o parecer da equipe PAF sobre a possibilidade de encaminhamento para financiamento.</p></div>
...
function ApplicationForm() {
  const initial = () => ({ clientId: crypto.randomUUID(), fullName: '', cpf: '', birthDate: '', phone: '', state: 'PA', consentVersion: LAND_CONSENT_VERSION, municipality: '', community: '', isFederalSettlement: null, motherName: '', settlementName: '', consent: false, website: '' });
  ...
  return <form ref={form} onSubmit={submit} className="land-form">
    ...
    <div className="land-fields" key={step}>
      {step === 0 && <>{field('fullName', 'Nome completo', { autoComplete: 'name', minLength: 5, maxLength: 160 })}<CpfField value={values.cpf} onChange={cpf => setValues(old => ({ ...old, cpf }))} />{field('phone', 'Telefone de contato com DDD', { type: 'tel', autoComplete: 'tel', maxLength: 20, placeholder: '(91) 99999-9999' })}<BirthDateField value={values.birthDate} onChange={birthDate => setValues(old => ({ ...old, birthDate }))} /></>}
...
function ReviewDialog({ id, api, onClose, onSaved }) {
  const dialog = useRef(null); const [data, setData] = useState(null); const [status, setStatus] = useState('EM_ANALISE'); const [comment, setComment] = useState(''); const [reviewerName, setReviewerName] = useState(''); const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  const [deleting, setDeleting] = useState(false); const [confirmation, setConfirmation] = useState('');
  const dirty = data && (status !== data.request.status || comment !== data.request.comment || reviewerName !== (data.request.reviewer_name || ''));
  function close() { if (!busy && (!dirty || window.confirm('Descartar o parecer ainda não salvo?'))) onClose(); }
  useEffect(() => { dialog.current.showModal(); const controller = new AbortController(); api(`${apiRoot}/admin/requests/${id}`, { signal: controller.signal }).then(result => { setData(result); setStatus(result.request.status); setComment(result.request.comment); setReviewerName(result.request.reviewer_name || ''); }).catch(err => { if (err.name !== 'AbortError') setError(err.message); }); return () => controller.abort(); }, [id, api]);
  async function save(event) {
    event.preventDefault(); if (busy) return; setBusy(true); setError('');
    try { const result = await api(`${apiRoot}/admin/requests/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status, comment, reviewerName, version: data.request.version }) }); setData(result); setStatus(result.request.status); setComment(result.request.comment); setReviewerName(result.request.reviewer_name || ''); onSaved(); }
    catch (err) { setError(err.message); } finally { setBusy(false); }
  }
  async function remove(event) {
    event.preventDefault(); if (busy || confirmation !== 'EXCLUIR') return;
    setBusy(true); setError('');
    try {
      await api(`${apiRoot}/admin/requests/${id}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ confirmation, version: data.request.version, protocol: data.request.protocol }) });
      onSaved(); onClose();
    } catch (err) { setError(err.message); } finally { setBusy(false); }
  }
  return <dialog className="land-dialog land-ui" ref={dialog} aria-labelledby="land-review-heading" onCancel={event => { event.preventDefault(); close(); }}><div className="land-dialog-header"><div><p className="eyebrow">ANÁLISE DA EQUIPE</p><h2 id="land-review-heading">{data?.request.full_name || 'Solicitação de análise'}</h2></div><button className="icon-text-button" type="button" title="Fechar análise" aria-label="Fechar análise" disabled={busy} onClick={close}><X size={20} /></button></div><div className="land-dialog-body"><Message>{error}</Message>{!data && !error && <p role="status">Carregando dados...</p>}{data && <><p className="land-protocol-small">{data.request.protocol}</p><dl className="land-data">{[['CPF', data.request.cpf], ['Data de nascimento', birthday(data.request.birth_date)], ['Telefone', data.request.phone || 'Não informado'], ['Município', data.request.municipality], ['Comunidade', data.request.community], ...settlementDetails(data.request.is_federal_settlement, data.request.mother_name, data.request.settlement_name), ['Recebimento', date(data.request.created_at)]].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><form onSubmit={save}><label>Nome de quem realizou a análise<input required minLength={3} maxLength={160} value={reviewerName} onChange={event => setReviewerName(event.target.value)} autoComplete="name" aria-describedby="land-reviewer-help" /></label><p id="land-reviewer-help" className="land-muted">Este nome aparecerá para o solicitante na consulta do andamento.</p><label>Resultado da análise<select value={status} onChange={e => setStatus(e.target.value)}>{Object.entries(LAND_STATUSES).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Parecer para o solicitante<textarea required minLength={10} maxLength={2000} rows={5} value={comment} onChange={e => setComment(e.target.value)} aria-describedby="land-comment-help" /></label><p id="land-comment-help" className="land-muted">Este comentário será visível na consulta pública. Registre o motivo e as orientações, sem dados bancários ou informações de terceiros.</p><div className="land-actions"><button className="primary-button" disabled={busy || !dirty}>{busy ? <Loader2 size={18} className="land-spin" /> : <Check size={18} />}{busy ? 'Salvando...' : 'Salvar parecer'}</button>{!dirty && data.history.length > 0 && <span role="status" className="land-muted">Parecer salvo e disponível para consulta.</span>}</div></form><h3>Histórico da solicitação</h3><History request={data.request} history={data.history} staff /><section className="land-delete-section" aria-label="Excluir solicitação">{!deleting ? <button type="button" className="icon-text-button land-delete-button" disabled={busy} onClick={() => setDeleting(true)}><Trash2 size={17} /> Excluir solicitação</button> : <form onSubmit={remove}><h3>Excluir esta solicitação?</h3><p>Você excluirá permanentemente a solicitação de <strong>{data.request.full_name}</strong>, protocolo <strong>{data.request.protocol}</strong>, e todo o histórico. O solicitante não poderá mais consultar esse protocolo. Alterações não salvas também serão descartadas.</p><label>Digite EXCLUIR para confirmar<input value={confirmation} onChange={event => setConfirmation(event.target.value)} required pattern="EXCLUIR" autoComplete="off" disabled={busy} /></label><div className="land-actions"><button type="button" className="icon-text-button" disabled={busy} onClick={() => { setDeleting(false); setConfirmation(''); }}>Cancelar exclusão</button><button className="primary-button land-delete-button" disabled={busy || confirmation !== 'EXCLUIR'}>{busy ? <Loader2 size={17} className="land-spin" /> : <Trash2 size={17} />} Excluir definitivamente</button></div></form>}</section></>}</div></dialog>;
}
